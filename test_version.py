...
def mvp(coins, limits, solana_investment):
    symbols = ['SOL', 'BTC', 'ETH', 'BNB', 'ADA', 'XRP']
    data = download_price_data_usd(symbols, 'day')
    prices = pd.DataFrame(data['SOL'])
    prices.rename(columns={'close': 'SOL'}, inplace=True)
    for s in symbols[1:5]:
        prices[s] = pd.DataFrame(data[s]).close
    prices.dropna(inplace=True)
    print('Your have chosen following coins: %s.' % coins)
    coins = coins.split(", ")
    ...
    print("\x1B[3m" + 'Please note that the maximum investment in one could can not exceed 50 %' + "\x1B[0m")
